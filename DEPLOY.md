...
> Faça este passo **depois** do Render, pois você precisa da URL do backend.

1. Acesse <https://vercel.com> → **Add New** → **Project** → importe o mesmo repo.
2. **Root Directory**: selecione **`client`** (o Vercel detecta o preset **Vite**).
   O `client/vercel.json` já cuida do fallback de SPA (rotas não dão 404 ao recarregar).
3. Em **Environment Variables**, adicione:

   | Variável | Valor |
   |---|---|
   | `VITE_API_URL` | a URL do Render, ex.: `https://mayra-api.onrender.com` (SEM `/api` no final) |

4. **Deploy**. Ao final, você terá uma URL como `https://atendente-client.vercel.app`.

> Importante: o `VITE_API_URL` é embutido no build. Se você mudar o valor depois,
> precisa clicar em **Redeploy** no Vercel para o frontend pegar a URL nova.
